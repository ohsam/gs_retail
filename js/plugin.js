    ...
    var t = {
            setupDOM: function(t) {
                if (!t.ptrElement) {
                    ...
                    t.mainElement !== document.body ? true : document.body.insertBefore(n, document.body.firstChild), n.classList.add(t.classPrefix + "ptr"), n.innerHTML = t.getMarkup().replace(/__PREFIX__/g, t.classPrefix), t.ptrElement = n, "function" == typeof t.onInit && t.onInit(t), e.styleEl || (e.styleEl = document.createElement("style"), e.styleEl.setAttribute("id", "pull-to-refresh-js-style"), document.head.appendChild(e.styleEl))
                    , e.styleEl.textContent = t.getStyles().replace(/__PREFIX__/g, t.classPrefix).replace(/\s+/g, " ")
                }
                return t
            },
            onReset: function(t) {
                t.mainElement.style.height = t.cssMarginTop+"px",
                setTimeout(function() {
            ...
            update: function(t) {
                ...
                if(e.state =='pending' || e.state =='refreshing'){
                    t.mainElement.classList.add('release')
                }else if(e.state =='pulling'){
                    t.mainElement.classList.remove('release')
                }
            }
        },
        n = function(t) {
        ...
        s = function() {
            ...
            function i(r) {
                //console MOVE
                s && s.ptrElement && e.enable && (e.pullStartY ? (e.pullMoveY = n(r))
                 : (s.shouldPullToRefresh() && (e.pullStartY = n(r))), 
                    "refreshing" !== e.state ? 
                    ("pending" === e.state && 
                        (e.state = "pulling", t.update(s)),
                         e.pullStartY && e.pullMoveY && (e.dist = e.pullMoveY - e.pullStartY),
                         e.distExtra = e.dist - s.distIgnore, 
                         e.distExtra > 0 && (r.cancelable && r.preventDefault(),
                         s.mainElement.style.height = s.cssMarginTop+ e.distResisted+"px",
                            e.distResisted = s.resistanceFunction(e.distExtra / s.distThreshold) * Math.min(s.distMax, e.distExtra), 
                            "pulling" === e.state && e.distResisted > s.distThreshold && (e.state = "releasing", t.update(s)), "releasing" === e.state && e.distResisted < s.distThreshold && 
                            (e.state = "pulling", t.update(s)))
                            ) : r.cancelable && s.shouldPullToRefresh() && e.pullStartY < e.pullMoveY && r.preventDefault())
            }
            function o() {
                if (s && s.ptrElement && e.enable) {
                    //console END
                    if ("releasing" === e.state && e.distResisted > s.distThreshold) e.state = "refreshing", 
                    s.mainElement.style.height = s.cssMarginTop + s.distReload + "px"
                    , e.timeout = setTimeout(function() {
                        ...
                        e.state = "pending",
                        //refresh하기 전 end 
                        s.mainElement.classList.add('release'),
                        s.mainElement.style.height = 0+"px"
        ...
        r = {
            ...
            getStyles: function() {
                return ".refreshArea.release{transition: height 0.3s}"
            },
            onInit: function() {},
            onRefresh: function() {
            ...
            shouldPullToRefresh: function() {
                if(window.scrollY<=0 && !e.overScroll){
                    return true;
                }else{
                    return false;
                }
            ...
            updateConsole: function(msg){
                return;
                document.querySelector('.debugClass').innerHTML+=msg+'<br/>';
                document.querySelector('.debugClass').scrollTop =100000;
            }
        },
